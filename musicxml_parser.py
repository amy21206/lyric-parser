...
#######
#
# Modifications made by amy21206
#
# Changes to the original file:
# Parses lyrics in notes.
#
######
...
class Note(object):
  ...
  def _parse(self):
    ...
    for child in self.xml_note:
      ...
      # ------ modified by amy21206 -------
      elif child.tag == 'lyric':
        if not self.has_lyric:
          self.has_lyric = True
          self.lyric = []
        lyric_number = child.attrib['number']
        for grandchild in child:
          if grandchild.tag == 'text':
            self.lyric.append({'number': lyric_number, 'text': grandchild.text})
      # ------ modified by amy21206 -------
      else:
        # Ignore other tag types because they are not relevant to Magenta.
        pass
  ...
  def get_pitch_and_lyric_list(self):
    res = []
  ...
  def __str__(self):
    ...
    if self.has_lyric:
      note_string += ', lyric: ' + str(self.lyric) + ' }'
